...
public class GenLayerSilurianSeaFix extends GenLayer
{
    ...
    public GenLayerSilurianSeaFix(long seed, GenLayer genLayer)
    {
        super(seed);
    ...
    private int[] diversify(int x, int z, int width, int height) {
        ...
        for (int zOut = 0; zOut < height; zOut++) {
            for (int xOut = 0; xOut < width; xOut++) {
                ...
                if (Biome.getBiome(center) == BiomeSilurianSeaShoreHelper.biome) {
                    output[i] = SILURIAN_OCEAN_SHORE_ID;
                } else {
                    output[i] = center;
                }
            }
