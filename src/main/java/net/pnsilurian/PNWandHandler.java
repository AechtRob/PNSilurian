...
import net.pnsilurian.world.dimension.silurian.WorldSilurian;
...
//        }
        ...
        if (levelEnchantment > 0) {
            ...
            if (world.isAirBlock(pos1)) {
                if (world.provider.getDimension() == LepidodendronConfig.dimSilurian) {
                    return;
                }
                boolean portalSpawnSilurian = WorldSilurian.portal.portalSpawn(world, pos1);

                if (portalSpawnSilurian) {
    ...
    public void goToNether(EntityTravelToDimensionEvent event) {
        if (LepidodendronConfig.oneWayPortalsNether && LepidodendronConfig.oneWayPortals) {
            if (event.getDimension() == -1) {
                ...
                if (event.getEntity().getEntityWorld().provider.getDimensionType().getId() == LepidodendronConfig.dimSilurian) {
                    if (!worldIn.isRemote && !entityIn.isRiding() && !entityIn.isBeingRidden() && entityIn instanceof EntityPlayerMP) {
                        ...
                        if (thePlayer.dimension != event.getDimension()) {
                            ...
                            WorldSilurian.BlockCustomPortal.transferPlayerToDimensionPN(thePlayer.server.getPlayerList(), thePlayer, event.getDimension(), WorldSilurian.BlockCustomPortal.getTeleporterForDimension(thePlayer, entityIn.getPosition(), event.getDimension()));
                        }
                    }
                    event.setCanceled(true);
                }
